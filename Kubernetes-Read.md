Here's the content of `575917933_1681528396.txt` formatted as Markdown (`.md`):
...
**Note:** PODS we can't access outside. We need to expose PODS for outside access using Kubernetes Service concept.
...
**Note:** As of now we have created PODS manually using POD manifest yml. If we delete our POD, K8S not re-creating the POD hence application will be down.
...
**Note:** It will take care of POD lifecycle. When POD is crashed / damaged / deleted then RC will create new pod.
...
11) Check in MySQL DB records are inserted or not
    ```bash
...
12) Exit from DB client & from DB container
    ```bash
    $ exit
    $ exit
    ```
...
- **Stateless applications** can't store the data permanently. For every request they will get new data and it will process that data. Every request will be treated as new request.
...
**Note:** By default prometheus and grafana services are available within the cluster as ClusterIP, to access them outside lets change it to NodePort / LoadBalancer.
...
**Note:** After practice completed delete ashokitns and efklog namespace.
...
- Metrics server is an application that collects metrics from objects such as pods, nodes according to the state of CPU, RAM and keeps them in time.
...
```bash
# Deploy two applications into K8S using Service using Cluster IP
